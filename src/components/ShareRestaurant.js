...
export default function ShareRestaurant({ restaurantName, restaurantId, style }) {
  const handleShare = useCallback(async () => {
    try {
      await Share.share({
        message: `Découvre ${restaurantName} sur Mida 🍽️\nmida://restaurant/${restaurantId}`,
...
const s = StyleSheet.create({
  ...
  icon:  { color: colors.accent, fontSize: typography.size.bodyLg || 15, fontWeight: typography.weight.bold },
